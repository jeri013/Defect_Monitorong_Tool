				...
				<g:each in="${defectInstanceList}" status="i" var="defectInstance">
					...
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>
							...
							<g:if test="${session?.user?.admin || isUser == 1}">
								
								<g:if test="${defectInstance?.defectStatus == 'To Do' || defectInstance?.defectStatus == 'On Going' || defectInstance?.defectStatus == 'Deploy to STG' || defectInstance?.defectStatus == 'Deploy to PROD'}">
